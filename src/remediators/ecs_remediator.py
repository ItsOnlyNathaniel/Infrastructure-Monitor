...
class ECSInstance:
    ...
    async def reboot_instance(self, cluster: str, service: str):
        ...
        logger.info(
            "Rebooting service %s in cluster %s by forcing new deployment",
            service,
            cluster,
        )
    ...
    async def restart_tasks(self, cluster: str, service: str):
        ...
        tasks_response = self.ecs.list_tasks(cluster=cluster, serviceName=service)
    ...
    async def rollback_deployment(self, cluster: str, service: str):  # Generated
        ...
        """
        logger.info(
            "Rolling back deployment for service %s in cluster %s", service, cluster
        )
        ...
        try:
            ...
            logger.info(
                "Task family: %s, Current revision: %s", task_family, current_revision
            )

            # Check if there's a previous revision
            if current_revision <= 1:
                raise ValueError(
                    "No previous revision available. Current revision is {current_revision}"
                )
            ...
            try:
                ...
                if e.response["Error"]["Code"] == "ClientException":
                    raise ValueError(
                        "Previous task definition %s not found: ", previous_task_def
                    )
                raise

            # Update service to use previous task definition
            logger.info(
                "Updating service to use task definition: %s", previous_task_def
            )
            ...
            logger.info(
                "Service update initiated. Deployment ID: %s",
                update_response["service"]["deployments"][0]["id"],
            )
            logger.info(
                "Successfully rolled back service %s to task definition %s",
                service,
                previous_task_def,
            )
            ...
            logger.error(
                "AWS API error during rollback: %s - %s", error_code, error_message
            )
